...
class UserAnswer(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    user_id = db.Column(db.Integer, db.ForrignKey('users.id'), nullable=False)
    questions_id = db.Column(db.Integer, db.ForrignKey('questions.id'), nullable=False)
    user_answer = db.Column(db.Integer, nullable=False)
    correctness = db.Column(db.Boolean, default=False)

    user_fk = db.relationship(User)
    questions_fk = db.relationship(Questions)
